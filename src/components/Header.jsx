    ...
    <header className="fixed top-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-b border-border z-50">
      <div className="container flex items-center justify-between">
        ...
        <nav className={`hidden md:flex items-center space-x-6`}>
          <a href="#about" className="nav-link-green">About Our Trading</a>
          <a href="#services" className="nav-link-green">Trading & Parts Services</a>
          <a href="#contact" className="inline-flex items-center justify-center gap-2 bg-gradient-hero text-primary-foreground hover:opacity-90 shadow-soft font-semibold h-9 rounded-md px-3 ml-4">Contact Trading Team</a>
        </nav>
      </div>
      {open && (
        <nav className="md:hidden container pb-4">
          <div className="rounded-lg border border-border bg-card p-2 shadow-card">
            <a className="block px-4 py-3 nav-link-green hover:bg-accent rounded-md" onClick={() => setOpen(false)} href="#about">About Our Trading</a>
            <a className="block px-4 py-3 nav-link-green hover:bg-accent rounded-md" onClick={() => setOpen(false)} href="#services">Trading & Parts Services</a>
            <a className="block px-4 py-3 bg-gradient-hero text-primary-foreground rounded-md font-semibold mt-2 text-center" onClick={() => setOpen(false)} href="#contact">Contact Trading Team</a>
          </div>
        </nav>
      )}
    </header>
